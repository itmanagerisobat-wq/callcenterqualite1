...
export default function SaisiePage() {
  ...
  async function ouvrirDossier(d) {
    setDossierOuvert(d);
    const { data } = await supabase
      .from("qualite_dossier_commentaires")
      .select("*")
      .eq("dossier_id", d.id)
  ...
  async function ajouterCommentaire(texte) {
    ...
    await supabase.from("qualite_dossier_commentaires").insert({
  ...
  async function soumettreDossier(form) {
    ...
    const { error } = await supabase.from("qualite_dossiers").insert(payload);
    if (error) {
