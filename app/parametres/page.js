...
const TABLES = [
  { key: "qualite_parametres_ingenieurs", label: "Ingénieurs", champ: "nom" },
  { key: "qualite_parametres_operations", label: "Opérations", champ: "libelle" },
  { key: "qualite_parametres_clients", label: "Clients", champ: "nom" },
  { key: "qualite_parametres_etats", label: "États", champ: "libelle" },
  { key: "qualite_parametres_validateurs", label: "Validateurs", champ: "nom" },
  { key: "qualite_parametres_causes_retour", label: "Causes de retour", champ: "libelle" },
];

export default function ParametresPage() {
  const { profile, loading, supabase } = useAppData();
  ...
  const [vue, setVue] = useState("listes"); // listes | objectifs | audit | backups
  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ data: objs }, { data: ings }] = await Promise.all([
        supabase.from("qualite_objectifs").select("*"),
        supabase.from("qualite_parametres_ingenieurs").select("nom").eq("actif", true),
      ]);
      setObjectifs(objs || []);
  ...
  async function ajouterLigne() {
    ...
    if (ongletActif === "qualite_parametres_causes_retour") payload.type = "generique";
  ...
  async function majObjectif(ingenieur, champ, valeur) {
    await supabase.from("qualite_objectifs").upsert(
      { ingenieur, [champ]: Number(valeur) || 0 },
      { onConflict: "ingenieur" }
    );
    const { data } = await supabase.from("qualite_objectifs").select("*");
    setObjectifs(data || []);
  }

  async function lancerSauvegardeManuelle() {
    const { data: doss } = await supabase.from("qualite_dossiers").select("*");
    await supabase.from("qualite_backups").insert({
      contenu: doss,
  ...
  function telechargerBackup(backup) {
    ...
    a.href = url;
    a.download = `isobat_backup_${backup.created_at?.slice(0, 10)}.json`;
    a.click();
  }
