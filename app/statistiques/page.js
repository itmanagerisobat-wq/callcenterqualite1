...
export default function StatistiquesPage() {
  const { profile, options, loading, supabase } = useAppData();
  ...
  // Répartition par état pour chaque ingénieur (bonus: par type d'opération)
  const etatsUniques = options.etats;
  const operationsUniques = options.operations;

  const dossiersARisque = filtres_.filter((d) => d.dossier_a_risque);
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="card p-4">
          <p className="text-xs font-semibold text-isoRed uppercase tracking-wide mb-3">
            Dossiers à risque — double retour (interne + client)
          </p>
          <ul className="flex flex-col divide-y text-sm">
            {dossiersARisque.map((d) => (
              <li key={d.id} className="py-2 flex justify-between">
                <span>{d.nom_dossier} — {d.ingenieur}</span>
                <span className="text-ink/40">{d.date}</span>
              </li>
            ))}
            {dossiersARisque.length === 0 && (
              <li className="py-4 text-center text-ink/40">Aucun dossier à risque. 👍</li>
            )}
          </ul>
        </div>
      </main>
