...
export default function ExportPage() {
  ...
  useEffect(() => {
    if (!profile) return;
    (async () => {
      const { data } = await supabase.from("qualite_dossiers").select("*").limit(5000);
      setDossiers(data || []);
    })();
  }, [profile]);
  ...
  function exporterExcel() {
    ...
    XLSX.writeFile(classeur, `ISOBAT_export_qualite_${new Date().toISOString().slice(0, 10)}.xlsx`);
  }
  ...
  async function importerFichier(e) {
    ...
    const { error, data } = await supabase.from("qualite_dossiers").insert(payload).select();
    setImportResume(
      error
