 ...
 *
 * NB: toutes les tables de cette app sont préfixées "qualite_" dans Supabase
...
export function useAppData() {
  ...
  const charger = useCallback(async () => {
    ...
    const [ing, ope, cli, eta, nat, causes, val] = await Promise.all([
      supabase.from("qualite_parametres_ingenieurs").select("nom").eq("actif", true).order("nom"),
      supabase.from("qualite_parametres_operations").select("libelle").eq("actif", true).order("libelle"),
      supabase.from("qualite_parametres_clients").select("nom").eq("actif", true).order("nom"),
      supabase.from("qualite_parametres_etats").select("libelle").eq("actif", true).order("ordre"),
      supabase.from("qualite_parametres_nature_production").select("libelle").eq("actif", true),
      supabase.from("qualite_parametres_causes_retour").select("libelle,type").eq("actif", true),
      supabase.from("qualite_parametres_validateurs").select("nom").eq("actif", true).order("nom"),
    ]);
  ...
  return { profile, options, loading, refresh: charger, supabase };
}
